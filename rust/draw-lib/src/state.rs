...
pub struct State {
    ...
    pub level: Level<DbKey>,

    pub bg_color: (u8, u8, u8),
    pub ground1_color: (u8, u8, u8),
    pub ground2_color: (u8, u8, u8),

    pub preview_object: GDObjectOpt,
    pub show_preview: bool,

    pub selected_object: Option<DbKey>,
    // select_depth: u32,
    pub show_collidable: bool,
    pub hide_triggers: bool,
    pub no_rotating_objects: bool,
    pub hide_grid: bool,
    pub hide_ground: bool,
    pub hide_outline: bool,
    pub select_hazards: bool,

    /// unix time, negative before event starts
    //pub event_elapsed: f64,
    pub event_start: f64,

    pub event_end: f64,
    pub ending_fully_done: f64,
    ...
    pub ending_anim_info: Option<EndingAnimInfo>,
    pub ending_transition_override: Option<f32>,
    pub ending_transition_speed: f32,
}
...
impl State {
    pub fn new(render: RenderState) -> Self {
        Self {
            ...
            now: 0.0, // default to client now before server now is gotten
...
impl State {
    ...
    pub fn add_object(&mut self, key: String, obj: GDObjectOpt) -> Result<(), RustError> {
        ...
        if let Ok(key) = key.into_bytes().try_into() {
            ...
            self.add_object_u8k(key, obj);
        }
        Ok(())
    }
    pub fn add_object_u8k(&mut self, key: [u8; 20], obj: GDObjectOpt) {
        self.level.add_object(obj.into_obj(), key, None, self.now);
    }

    pub fn delete_object(&mut self, key: String) -> Option<Vec<f32>> {
        if let Ok(key) = key.into_bytes().try_into() {
            self.delete_object_u8k(key)
    ...
    pub fn delete_object_u8k(&mut self, key: [u8; 20]) -> Option<Vec<f32>> {
        let key: DbKey = key;
