...
<div class="absolute text-white">
    {#each Object.entries($deleteTexts) as [k, { name, x, y }] (k)}
        ...
        >
            <span
                class="absolute letext text-black text-saira text-stroke-both text-3xl text-nowrap w-[500px] text-center"
...
<style>
    @keyframes example {
        0% {
            transform: translate(-50%, calc(0px - 50%)) scale(0.5);
            opacity: 100%;
        }
        80% {
            transform: translate(-50%, calc(-30px - 50%)) scale(0.5);
            opacity: 0%;
        }
        100% {
            transform: translate(-50%, calc(-30px - 50%)) scale(0.5);
