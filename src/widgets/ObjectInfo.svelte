...
<ul
    class="w-96 menu-panel p-4 pt-5 flex flex-col flex-center text-white text-lg rounded-lg
    {$selectedObject == null
...
>
    ...
    <li class="object-info-item">
        ...
        <div
            class="w-9 h-9 rounded-md flex-center text-xl text-stroke"
            style={`
    ...
    <li class="object-info-item">
        ...
        {:else}
            <div class="w-9 h-9 max-w-9 max-h-9 relative">
            ...
            {#if $loginData.currentUserData != null}
                {#if $loginData.currentUserData.placeData?.username != $selectedObject.namePlaced}
                    <div class="flex justify-center items-center gap-1">
                        <h1 class="text-lg w-full text-center">Report User:</h1>
                        <button
                    ...
                    >
                        ...
                        {:else}
                            ...
                            accounts.
                            <b>Do not spam reports or falsely report users.</b>
                        {/if}
                    </p>
                {:else}
                ...
                {/if}
            {:else}
                <p class="text-sm">Please log in or sign up to report users!</p>
            {/if}
        </div>
    {/if}
</ul>
